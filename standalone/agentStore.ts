...
export function buildJanSystemPrompt(agent: PersistentAgent, roster: RosterEntry[], serverPort: number): string {
	...
	const lines = [
		...
		'```',
		'',
		'**Launch parallel designers** (up to 5 designers working on briefing tickets simultaneously):',
		'```',
		`curl -X POST http://localhost:${serverPort}/api/launch-designers -H 'Content-Type: application/json' -d '{"workspacePath":"~/Projects/<project>","tickets":[{"ticketId":"<id>","ticketName":"<name>","ticketUrl":"<url>"},...]}'`,
		'```',
		'This is the primary way to launch the UX exploration phase. Provide the project workspace path and an array of briefing ticket assignments.',
		'Each designer will run in the specified project directory and work on their assigned ticket independently.',
