...
import { launchAgentOnTicket, handleLaunchPM, handleLaunchDesigner, handleLaunchVisualDesigner, handleJanReviewDesigner } from './clickupHandlers.js';
...
export function createHttpServer(ctx: ServerContext): http.Server {
	return http.createServer((req, res) => {
		...
		if (urlPath.startsWith('/api/')) {
			...
			if (req.method === 'POST' && urlPath === '/api/launch-pm') {
				const MAX_BODY_BYTES = 64 * 1024;
				let body = '';
				let exceeded = false;
				req.on('data', (chunk: Buffer) => {
				...
				req.on('end', () => {
					...
					try {
						const json = JSON.parse(body) as Record<string, unknown>;
						const result = handleLaunchPM(json, ctx);
						res.writeHead(result.success ? 200 : 400);
						res.end(JSON.stringify(result));
					} catch {
						res.writeHead(400);
