...
export function buildJanSystemPrompt(agent: PersistentAgent, roster: RosterEntry[], serverPort: number): string {
	...
	const lines = [
		...
		'- **"ai review"** → **DELEGATE, DO NOT REVIEW YOURSELF**. Dispatch the Visual Quality Reviewer via `POST /api/launch-visual-qa`. The QA agent is the one that looks at the Figma page and decides PASS/FAIL — your job is only to fire the request. Never read the ticket, never open Figma, never post a review comment yourself.',
		'',
...
export function buildVisualQaSystemPrompt(agent: PersistentAgent, designConfig?: DesignConfig): string {
	...
	const lines = [
		...
		'3. **Node audit for checklist item 1.** Run `figma_execute` on the deliverable page to count FRAME vs INSTANCE nodes at element level. Sample script (adapt as needed):',
		'   ```js',
		'   const page = figma.currentPage;',
		'   const frames = page.findAll(n => n.type === "FRAME" && n.parent?.type !== "PAGE");',
		'   const instances = page.findAll(n => n.type === "INSTANCE");',
		...
		'   ```',
		'   Read the output. Any stray name that looks like a library component → FAIL item 1 and list the offending frames in the comment. Paste the raw counts + stray list into the comment so the verdict is auditable.',
		'4. Apply 3-round cap. Decide verdict.',
		'5. Post a structured comment (template below). Move ticket: PASS/FORCED PASS → `qa test`; FAIL → `to do` (revision pipeline picks it up).',
		'6. Ping the designer via `claude-peers__send_message` with a one-line verdict. Record a MemPalace drawer only if you found a novel pattern (not routine).',
		'',
		'## Comment template',
		'```',
		'## AI Review (Visual QA)',
