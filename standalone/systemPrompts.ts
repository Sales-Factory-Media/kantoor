...
export function buildVisualQaSystemPrompt(agent: PersistentAgent, designConfig?: DesignConfig): string {
	...
	const lines = [
		...
		'4. **Node audit for checklist item 1.** Run `figma_execute` on the deliverable page to count FRAME vs INSTANCE nodes at element level. Sample script (adapt as needed):',
		'   ```js',
		'   const page = figma.currentPage;',
		'   const frames = page.findAll(n => n.type === "FRAME" && n.parent?.type !== "PAGE");',
		'   const instances = page.findAll(n => n.type === "INSTANCE");',
		...
		'5. Apply 3-round cap. Decide verdict.',
		'6. Post a structured comment (template below). Move ticket: PASS/FORCED PASS → `qa test`; FAIL → `to do` (revision pipeline picks it up). (Ticket is currently `in progress` from step 1 — now moving OUT of `in progress` to the final status.)',
		'7. Ping the designer via `claude-peers__send_message` with a one-line verdict. Record a MemPalace drawer only if you found a novel pattern (not routine).',
		'',
		'## Comment template',
		'```',
		'## AI Review (Visual QA)',
